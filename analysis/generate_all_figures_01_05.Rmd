...
rm(list=ls())
setwd("~/Desktop/reznik/gel_paper/analysis/")
...
## load data
gel_hotspots        <- read.csv("../data/hotspots_snv_0223.txt", header = TRUE, sep = "\t") %>%
                       subset(q.value < 0.05)
...
gel_mutations       <- read.csv("../data/mtvariantcounts_0226.txt", sep = "\t", header = TRUE) 
...
helix_muts          <- read.csv("../data/helix_mutations.csv", sep = ",", header = TRUE, row.names=1)  %>% 
                       subset(variant_type == "SNP") 

allpos              <- data.frame(1:16569)
colnames(allpos)    <- c('position')

annotations         <- read.csv("../data/mito_rRNA_annotations_NL.tsv", sep = "\t")

pcawg_mutations     <- read.csv("../data/data_mutations_pcawg.txt", sep = "\t", header = TRUE) %>% 
                       subset(Variant_Type == "SNP")
pcawg_hotspots      <- read.csv("../data/hotspots_snv_pcawg.txt", sep = "\t", header = TRUE) %>%
                       subset(q.value < 0.05)

tcga_mutations      <- read.csv("../data/data_mutations_tcga.txt", sep = "\t", header = TRUE) %>% 
                       subset(Variant_Type == "SNP")
tcga_hotspots       <- read.csv("../data/hotspots_snv_precalculated.txt", sep = "\t", header = TRUE) %>%
                       subset(q.value < 0.05)

pathogenic_pc       <- read.csv("../data/MitImpact_db_3.1.2.txt", sep = "\t", header = TRUE)
...
if(get_pdfs==T) ggsave('../figures_v2/panel_1b.pdf',width=4,height=2.5)
...
if(get_pdfs==T) ggsave('../figures_v2/panel_1c.pdf',width=2,height=2.5)
```
...
if(get_pdfs==T) ggsave('../figures_v2/panel_1e.pdf',width=2.5,height=2.5)
...
p.value
if(get_pdfs==T) ggsave('../figures_v2/panel_2e.pdf',width=2.5,height=2)
...
p.value$est
if(get_pdfs==T) ggsave('../figures_v2/panel_2h.pdf',width=2.5,height=2)
```
## 2i
```{r panel_2h, fig.height = 2, fig.width = 2, eval = TRUE}
...
p.value
if(get_pdfs==T) ggsave('../figures_v2/panel_2i.pdf',width=2.5,height=2)
```
...
if(get_pdfs==T) ggsave('../figures_v2/supp_2a.pdf',width=3.5,height=3)
```
...
if(get_pdfs==T) ggsave('../figures_v2/supp_2b.pdf',width=3.5,height=3)
```
...
rnr_foswiki               <- read.csv("../data/MutationsRNACfrm  MITOMAP  Foswiki.csv", sep = ",", header = TRUE,   check.names = TRUE)  %>% 
                             select(Position, Locus)

pc_foswiki                <- read.csv("../data/ConfirmedMutations  MITOMAP  Foswiki.csv", sep = ",", header = TRUE, check.names = TRUE) %>% 
                             select(Position, Locus)
...
# save file
ggsave(barplot_mitomap_known_disease_causing, file = "../figures_v2/supp_3a.pdf", width = 2, height = 2)
